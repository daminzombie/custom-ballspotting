...
from custom_ballspotting.actions import Action, NUM_ACTION_CLASSES
from custom_ballspotting.checkpoints import read_checkpoint_metadata
from custom_ballspotting.data import (
    CustomTDeedDataset,
    VideoRecord,
)
from custom_ballspotting.model.tdeed import CustomTDeedModule, action_probabilities
...
DEFAULT_DECODE_THRESHOLDS: dict[str, float] = {
    ...
    Action.BALL_OUT_OF_PLAY_CLEAR.value: 0.45,
    Action.BALL_OUT_OF_PLAY_DISTANT.value: 0.45,
...
DEFAULT_DECODE_NMS_WINDOW_FRAMES: dict[str, int] = {
    ...
    Action.BALL_OUT_OF_PLAY_CLEAR.value: 12,
    Action.BALL_OUT_OF_PLAY_DISTANT.value: 12,
...
def resolve_infer_video_params(
    ...
    *,
    ...
    if meta is not None:
        ...
        if head_type == "separate_action_team":
            raise ValueError(
                "This checkpoint uses the retired two-head architecture "
                "(head_type='separate_action_team'). Retrain with the current "
                "action-only model or load backbone weights only."
            )
        if head_type is not None and head_type != "action_only":
            raise ValueError(
                f"Checkpoint head_type={head_type!r} is not compatible with the "
                "action-only model. Use a checkpoint saved by this package with "
                "head_type='action_only'."
            )
        if meta.get("num_team_action_classes") is not None:
            _logger.warning(
                "Checkpoint metadata still lists num_team_action_classes; "
                "ignoring it for action-only inference.",
            )
    else:
        _logger.warning(
    ...
    with torch.no_grad():
        for batch in tqdm(loader, total=len(loader), desc="scoring", disable=not sys.stderr.isatty()):
            ...
            with torch.amp.autocast(device_type=device, enabled=device == "cuda"):
                ...
                probs = action_probabilities(outputs).detach().cpu().numpy()
...
def scores_to_predictions(
    ...
    for action_idx, action in enumerate(Action):
        class_scores = scores[:, action_idx + 1]
